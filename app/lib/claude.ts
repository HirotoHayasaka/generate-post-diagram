...
- 必要に応じてスクロールバーを表示せず、すべてのコンテンツが1画面内に収まるようにしてください
- 内容が多い場合は、情報を簡略化するか、最も重要な要素のみを表示してください

## テキストと可読性
- 文字サイズは非常に小さめに設定してください（10px〜12pxを基本とし、見出しでも最大14px）
- 特に強調：すべてのテキスト要素は小さく控えめにしてください。大きな文字は使用しないでください
- ただし、メインタイトルのみは例外として16px〜18px程度の大きさを保ち、視覚的な階層を作ってください
- 背景色は白または淡い色を使用し、テキストは暗い色で十分なコントラストを確保してください
- テキスト要素には必ず適切な余白（padding: 4px〜8px程度）を設定し、読みやすさを確保してください
- 行間（line-height: 1.2〜1.4程度）を適切に取り、テキストの可読性を高めつつ縦スペースを節約してください
- 段落や項目間には最小限のマージン（margin: 4px〜6px程度）を設定し、コンテンツの区切りを明確にしつつスペースを節約してください
- 長いテキストは箇条書きや短い文に分割し、縦方向のスペースを節約してください

## デザイン要素
- 図解内の要素は最小限の余白を持たせ、全体的にコンパクトなレイアウトにしてください
- 必ず16:9のアスペクト比を維持し、内容量はそれに合わせてください。オーバーするようであれば重要な箇所を残し、それ以外は削除してください
- 鮮明で高解像度の表示になるよう、すべての要素に明確なボーダーと十分なコントラストを持たせてください
- モダンなデザイン要素として、控えめなグラデーション、シャドウ効果、丸みを帯びた角などを適切に使用してください
- アイコンや図形は非常に小さめで洗練されたものを使用し、全体の調和を保ってください
- 全体的に小さく、コンパクトな要素を使用し、情報密度の高い図解を目指してください
- 縦方向のスペースを節約するため、横方向のレイアウトを優先してください

## 縦方向のスペース節約のための追加指示
- グリッドレイアウトを使用する場合は、縦方向の行数を最小限に抑え、横方向の列を優先してください
- 余白やパディングは横方向より縦方向を小さくしてください（例：padding: 4px 8px;）
- 高さが固定されたコンテナを使用し、その中でコンテンツを適切に配置してください
- 縦方向に伸びる要素（リストなど）は横方向に並べるか、複数列に分割してください
- 内容が多い場合は、情報の優先順位を付け、重要度の低い情報は省略してください

## shadcn/uiスタイルガイドライン
- カード、ボタン、アイコンなどの要素はshadcn/uiのようなミニマルでモダンなデザインに
- 色彩はshadcn/uiのようなニュートラルな色調をベースに、アクセントカラーを控えめに使用
- 要素の角は適度に丸く（border-radius: 4px〜6px程度）
- シャドウは非常に軽く（box-shadow: 0 1px 2px rgba(0,0,0,0.05)程度）
- ボーダーは薄く（1px solid rgba(0,0,0,0.1)程度）

下記は参考にして欲しいグラフィックレコーディングの例です。ただし、この例よりもさらにコンパクトで、16:9のアスペクト比に収まるようにしてください。
...
レスポンスは<div>タグから始まるHTMLコードのみを返してください。コメントや説明は含めないでください。また、必ず16:9のアスペクト比に収まるようにしてください。縦方向にはみ出さないよう、コンテンツ量を調整してください。`,
          },
        ],
      }),
    });
